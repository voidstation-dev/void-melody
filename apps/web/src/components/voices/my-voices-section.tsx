...
import { OmniVoiceDesignCard } from "./omnivoice-design-card"
import { VoiceLibraryEmpty } from "./voice-library-empty"
import { VoiceLibrarySkeleton } from "./voice-library-skeleton"

type MyVoicesSectionProps = { voices: CustomVoice[]; total: number; page: number; pageSize: number; isLoading: boolean; isError: boolean; hasFilters: boolean; deletingVoiceId?: string; onRetry: () => void; onDelete: (voiceId: string) => void; onPageChange: (page: number) => void; onCreateVoice: () => void }

export function MyVoicesSection({ voices, total, page, pageSize, isLoading, isError, hasFilters, deletingVoiceId, onRetry, onDelete, onPageChange, onCreateVoice }: MyVoicesSectionProps) {
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex min-h-9 items-center gap-1 rounded-xl bg-primary px-3.5 py-2 text-xs font-bold text-primary-foreground shadow-2xs transition-opacity hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
          >
            <Plus className="h-3.5 w-3.5" />{t("voices.newVoice")}
          </button>
        </div>
      </div>
      {isLoading ? <VoiceLibrarySkeleton variant="custom" /> : isError ? <div className="rounded-2xl border border-destructive/30 bg-destructive/5 px-5 py-6 text-sm"><p className="font-bold text-destructive">{t("voices.customLoadError")}</p><button type="button" onClick={onRetry} className="mt-3 inline-flex items-center gap-1 text-xs font-bold text-destructive underline underline-offset-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><RefreshCcw className="h-3.5 w-3.5" />{t("common.retry")}</button></div> : voices.length === 0 ? <VoiceLibraryEmpty kind={hasFilters ? "search" : "custom"} /> : <div className="grid gap-3 sm:grid-cols-2">{voices.map((voice) => {
        const isOmni = voice.provider_id === "omnivoice" && voice.profile_format_version?.startsWith("omnivoice")
        return isOmni
          ? <OmniVoiceDesignCard key={voice.id} voice={voice} onDelete={onDelete} deleting={deletingVoiceId === voice.id} />
          : <CustomVoiceCard key={voice.id} voice={voice} onDelete={onDelete} deleting={deletingVoiceId === voice.id} />
      })}</div>}

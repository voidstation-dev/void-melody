...
export function VieneuPage({ initialVoiceId = null }: { initialVoiceId?: string | null } = {}) {
  ...
  const generatePreview = async () => {
    ...
    try {
      ...
      setPreviewError(readableError(error, "Preview could not be queued."))
    }
  }

  return <div className="flex h-full min-h-0 flex-col">
    <header className="mb-5 flex shrink-0 flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
      <div><p className="mb-1 text-[11px] font-bold uppercase tracking-[0.18em] text-primary/60">VieNeu / local audio</p><h1 className="text-2xl font-bold tracking-tight">Voice Lab</h1><p className="mt-1 max-w-xl text-sm text-muted-foreground">Create reusable voice profiles from a short reference sample. Your audio stays on this device.</p></div>

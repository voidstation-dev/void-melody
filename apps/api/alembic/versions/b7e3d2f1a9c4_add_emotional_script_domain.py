...
def upgrade() -> None:
    ...
    if "emotional_scripts" not in existing_tables:
        op.create_table(
            "emotional_scripts",
    ...
    if "script_renders" not in existing_tables:
        op.create_table(
            "script_renders",
            sa.Column("id", sa.String(36), nullable=False),
    ...
    if "script_render_segments" not in existing_tables:
        op.create_table(
    ...
    if "script_audio_cache" not in existing_tables:
        op.create_table(
            "script_audio_cache",
